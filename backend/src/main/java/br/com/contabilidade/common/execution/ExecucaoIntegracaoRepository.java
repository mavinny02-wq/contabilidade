...
public interface ExecucaoIntegracaoRepository extends JpaRepository<ExecucaoIntegracao, UUID> {
    ...
    Page<ExecucaoIntegracao> findByEmpresaIdOrderByCriadoEmDesc(UUID empresaId, Pageable pageable);
    long countByStatusIn(Collection<StatusExecucao> statuses);
    Optional<ExecucaoIntegracao> findByIdempotencyKey(String idempotencyKey);
}
